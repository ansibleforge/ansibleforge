...
Each user gets an isolated workspace running the `tools-ansibleforge` and `ee-ansibleforge` containers, with Vault secrets injected automatically and persistent storage attached.
...
The workspace definition lives in `ansibleforge/ansibleforge` at `devspaces-template/devfile.yaml`.
...
| Container | Image |
|-----------|-------|
| `development-container` | `shared-builds/tools-ansibleforge:latest` |
